...
use crate::behaviour::{DendriteBehaviour, DendriteBehaviourEvent};
...
impl Libp2pTransport {
    ...
    fn subscribe_all(&mut self) -> Result<()> {
        for topic in gossip::dendrite_topics() {
    ...
    pub async fn next_event(&mut self) -> NetworkEvent {
        loop {
            match self.swarm.select_next_some().await {
                SwarmEvent::Behaviour(DendriteBehaviourEvent::Gossipsub(
                ...
                SwarmEvent::Behaviour(DendriteBehaviourEvent::Mdns(mdns::Event::Discovered(
                    peers,
                ))) => {
                ...
                SwarmEvent::Behaviour(DendriteBehaviourEvent::Mdns(mdns::Event::Expired(
                    peers,
                ))) => {
